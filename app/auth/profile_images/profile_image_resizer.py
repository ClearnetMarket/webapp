        ...
        for file in files:
            if file.endswith(tuple(ext)):
                ...
                if file.endswith("-unknown.png"):
                    ...
                    if file.endswith("_125x.jpg"):
                        pass
                    else:
                        # see if width greater than 125
                        y = testsize(newfullpathfilename)
                        if y == 1:
                            pass
                        else:
                            # CREATE A 125 size image
                            # opens the image
                            img = Image.open(newfullpathfilename)
                            # gets base name
                            thebasename = os.path.splitext(file)[0]
                            newname_125 = thebasename + "_125x"
                            renamed_file = (os.path.join(root, newname_125 + extension))
